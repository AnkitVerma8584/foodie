...
class ProductRecyclerAdapter(val context: Context,private val productList: ArrayList<Products>) :
    RecyclerView.Adapter<ProductRecyclerAdapter.HomeViewHolder>() {
    ...
    private fun statusProduct(status: String, pid: String) {

        val queue = Volley.newRequestQueue(context)
        val url = "https://admin.manadelivery.in/admin/products_status/"
        val jsonParams = JSONObject()

        val uid = "&product_id=$pid"
        val ustatus = "?stock=$status"
        jsonParams.put("product_id", pid)
        jsonParams.put("stock", status)
        val jsonObjectRequest = object :
            JsonObjectRequest(Method.POST, url + ustatus + uid, jsonParams, Response.Listener {
                try {
